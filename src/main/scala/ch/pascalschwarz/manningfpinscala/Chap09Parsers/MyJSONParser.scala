...
object MyJSONParser {
  def jsonParser[Parser[+_]](P: Parsers[Parser]): Parser[JSON] = {
    ...
    val jsonBool: Parser[JBool] = ("true" | "false").token map (b => JBool(b.toBoolean))


    object stringParsing {
      val unicode:Parser[String] = ("\\u" *> "[0-9a-fA-F]{4}".r) map (u => Character.toChars(Integer.parseInt(u, 16)).head.toString)
